...
public class Main {
	...
	static Pattern pTestFunctions = Pattern.compile(
			"\\bvoid\\s++(?=[a-zA-Z])(?<functionName>(?>\\w*?test\\w*))\\s*+\\(\\s*+\\)\\s*+(?<functionBody>\\{(//.*+(\r)?\n|/\\*[\\s\\S]*?\\*/|\"(?:\\\\.|[^\"\\\\]++)*+\"|[^\"{}/]++|(?functionBody))*+\\})");

	static Pattern pTestRegexSearch = Pattern.compile(
	...
	static Pattern pInvalidRegex = Pattern.compile(
			"/\\*[\\s\\S]*?\\*/|//.*(\r)?\n|TEST_INVALID_REGEX\\s*\\(\\s*(?<regex>(?<javaString>\"(\\\\.|[^\"])*\"))\\s*,\\s*(?<options>((perl|icase|nosubs|no_mod_m|no_mod_s|mod_s|mod_x)\\s*(\\||(?=\\))))+)\\)\\s*;");

	private static final File dir = new File("boostregextests");

	private static URL url;

	private static final String[] testFiles = new String[] { "basic_tests.cpp", "test_tricky_cases.cpp", "test_alt.cpp",
			"test_simple_repeats.cpp", "test_perl_ex.cpp", "test_non_greedy_repeats.cpp", "test_grep.cpp",
			"test_backrefs.cpp" };
	...
	public static void main(String[] args) {
		try {
			if (!dir.exists() && !dir.mkdir())
				System.exit(1);
			url = new URL("https://github.com/boostorg/regex/raw/develop/test/regress");
			...
			for (String testFile : testFiles) {
				...
				File f = downloadFile(testFile);

				String cpp_file_content = IOUtils.toString(new FileInputStream(f));
				...
				while (mTestFunctions.find()) {
					if (mTestFunctions.group("functionName").equals("test_verbs"))
						continue;
					...
					while (m.find()) {
						if (!m.group().startsWith("TEST"))
							continue;
						if (!isValidArray(m.group("array")))
							continue;
						...
						sbFunctions.append(",").append(m.group("input")).append(", new int[]{").append(array)
								.append("});\n");
					}
					/*
					 * Matcher m2 = pInvalidRegex.matcher(functionBody);
					 * 
					 * while(m2.find() ){ if(! m2.group().startsWith("TEST") )
					 * continue; String regex =
					 * Main.adaptRegex(m2.group("regex") ); if(regex == null)
					 * continue;
					 * sbFunctions.append("\t\tcheckExpectedFail(").append(
					 * regex ).append(","); appendOptions(m2.group("options"),
					 * null, sbFunctions ); sbFunctions.append(");\n"); }
					 */
	...
	private static File downloadFile(String testFile) throws MalformedURLException, IOException {
		File f = new File(dir, testFile);
		if (!f.exists()) {
			ReadableByteChannel rbc = Channels.newChannel(new URL(url.toString() + "/" + testFile).openStream());
	...
	private static void appendOptions(String options, String otherOptions, StringBuilder sb) {
		...
		if (pModS.matcher(options).find() || (!options.contains("no_mod_s")
				&& (otherOptions == null || !otherOptions.contains("match_not_dot_newline"))))
	...
	static String adaptRegex(String regex) {
		if (isSpecialRegex(regex))
			return null;
		if (regex.contains("(?i:(?1)"))
			return null;
		if (regex.contains("(?:(?<A>a+)|(?<A>b+)"))
			return null;
		if (containsRelativeBackReference(regex))
			return null;
		if (containsUnsupportedRecursion(regex))
			return null;
		if (containsUnsupportedConditional(regex))
			return null;
		if (containsBranchReset(regex))
			return null;
		if (regex
				.equals("\"^\\\\W*(?:((.)\\\\W*(?1)\\\\W*\\\\2|)|((.)\\\\W*(?3)\\\\W*\\\\4|\\\\W*.\\\\W*))\\\\W*$\"")) {
		...
		if (containsRecursionToGroupThatsNotYetDeclared(regex))
			return null;
		return regex;
	}
	...
	private static final Pattern pWrongBackReference = Pattern
			.compile(NO_ESCAPE + "\\\\{2}g((?<groupNumber>\\d+)|\\{(?groupNumber)\\})");
	private static final Pattern pWrongBackReference2 = Pattern
			.compile(NO_ESCAPE + "\\\\{2}g\\{(?<groupName>[a-zA-Z][a-zA-Z0-9]*)\\}");
	...
	private static final Pattern pWrongNamedGroups = Pattern
			.compile(NO_ESCAPE + "\\(\\?'(?<groupName>[a-zA-Z][a-zA-Z0-9]*)'");
	...
	private static String adaptWrongCommentaries(String regex) {
		...
		return m.replaceAll((Matcher matcher) -> {
			return "(?x:#" + matcher.group("comment").replaceAll("\\\\n", "") + "\\n)";
		});
	}
	...
	private static String adaptWordBoundaries(String regex) {
		return pWrongWordBoundaries.matcher(regex).replaceAll((Matcher matcher) -> {
			return "\\\\b";
		});
	}
	...
	static {
		...
		pGreaterAndSmallerSigns = Pattern.compile(
				NO_ESCAPE + "(" + namedGroup + "|" + conditionBasedOnValidGroupCapture + "|" + backReference + "|"
						+ characterClass + "|" + lookbehind + "|" + independentGroup + "|" + recursion + "|[><])");
	}
	...
	private static final Pattern pWrongRecursionGroups = Pattern
			.compile("(\\d+\\s*,\\s*\\d+\\s*,)([\\s\\S]*?)(-\\s*2)");

	private static String adaptWrongRecursionGroups(String array) {
		return pWrongRecursionGroups.matcher(array).replaceAll("$1$3");
	}

	private static final Pattern pWrongRecursion = Pattern
			.compile(NO_ESCAPE + "\\(\\?(&|P\\>)(?<groupName>[a-zA-Z][a-zA-Z0-9]*)\\)");

	private static String adaptWrongRecursion(String regex) {
		return pWrongRecursion.matcher(regex).replaceAll("(?${groupName})");
	}

	private static final Pattern pWrongConditionalBasedOnValidGroupCapture = Pattern
			.compile(NO_ESCAPE + "\\(\\?\\((?:'|(?<cond>[<]))(?<groupName>[a-zA-Z][a-zA-Z0-9]*)(?(cond)[>]|')\\)");
	...
	private static final Pattern pUnsupportedConditional = Pattern.compile(NO_ESCAPE
			+ "\\Q(?(\\E(\\?\\!|DEFINE(?=\\))|\\Q?<=\\E|\\Q?<!\\E|R&(?<groupName>[a-zA-Z][a-zA-Z0-9]*)(?=\\))|R\\d+(?=\\)))");

	private static boolean containsUnsupportedConditional(String regex) {
		return pUnsupportedConditional.matcher(regex).find();
	}

	private static final Pattern pCharacterClasses = Pattern.compile(NO_ESCAPE
			+ "\\[(?<not>\\^)?\\[:(?<class>lower|upper|alpha|digit|alnum|punct|graph|print|blank|cntrl|space):\\]\\]");
	private static final Pattern pWordAndXDigitCharacterClasses = Pattern
			.compile(NO_ESCAPE + "\\[(?<not>\\^)?\\[:(?<class>word|xdigit):\\]\\]");
	...
	private static String adaptCharacterClasses(String regex) {
		...
		regex = m.replaceAll((Matcher matcher) -> {
			...
			String r = "\\\\p{" + Character.toUpperCase(matcher.group("class").charAt(0))
					+ matcher.group("class").substring(1) + "}";
	...
	private static boolean containsRecursionToGroupThatsNotYetDeclared(String regex) {
		if (containsRecursionToNumberedGroupThatsNotYetDeclared(regex))
			return true;
		if (containsRecursionToNamedGroupThatsNotYetDeclared(regex))
			return true;
		return false;
	}

	private static final Pattern pRecursionToNumberedGroup = Pattern
			.compile(NO_ESCAPE + "(\\(\\?(?<groupNumber>\\d+)\\)|\\Q(?(\\E(?groupNumber)\\))");
	private static final Pattern pGroupCounter = Pattern
			.compile("^(" + NO_ESCAPE + "(?<capturingGroup>\\((?:\\?\\<[a-zA-Z][a-zA-Z0-9]*\\>|(?!\\?)))|[\\s\\S])*$");
	...
	private static boolean isGroupAlreadyDeclared(int group, String subregex) {
		...
		if (!m.find())
			throw new RuntimeException("The regex should always match!");
		return m.captures("capturingGroup").size() >= group;
	}

	private static final Pattern pRecursionToNamedGroup = Pattern
			.compile(NO_ESCAPE + "(\\(\\?(?<groupName>[a-zA-Z][a-zA-Z0-9]*)\\)|\\Q(?(\\E(?groupName)\\))");
	...
	private static final String[] SPECIAL_PATTERNS = new String[] { "\"\\\\b\"", "\"^a (?#xxx) (?#yyy) {3}c\"",
			"\"^   a\\\\ b[c ]d       $\"", "\"((?-i)[[:lower:]])[[:lower:]]\"", "\"^\"", "\"\\\\<\"", "\"[a-Z]+\"",
			"\"[[:lower:]]+\"", "\"[[:upper:]]+\"", "\"\\\\l+\"", "\"\\\\u+\"", "\"a{ 2 , 4 }\"", "\"a{ 2 , }\"",
			"\"a{ 2 }\"", "\"a{12b\"" };

	private static boolean isSpecialRegex(String regex) {
		for (int i = 0; i < SPECIAL_PATTERNS.length; ++i) {
			if (SPECIAL_PATTERNS[i].equals(regex))
				return true;
		}
		return false;
	}
	...
	private static final Pattern pValidArray = Pattern.compile(
			"(^|(?<=-\\s{0,10}2\\s{0,10},))\\s*(?<first>\\d+)\\s*,\\s*\\k<first>\\s*,[\\s\\S]*?-\\s*2\\s*,\\s*\\k<first>");
